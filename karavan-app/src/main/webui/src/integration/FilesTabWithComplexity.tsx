...
import {FilesToolbar} from "@/integration/files/FilesToolbar";
import {UploadFileModal} from "@/integration/files/UploadFileModal";
import {DeleteFileModal} from "@/integration/files/DeleteFileModal";
import {DiffFileModal} from "@/integration/files/DiffFileModal";
import {CreateFileModal} from "@/integration/files/CreateFileModal";
...
import {JKubeIcon} from "@/integration-designer/icons/KaravanIcons";
...
import {ComplexityProject} from "@/integrations/ComplexityModels";
import {ComplexityApi} from "@/integrations/ComplexityApi";
import {SvgIcon} from "@/icons/SvgIcon";
import {RenameFileModal} from "@/integration/RenameFileModal";
...
export function FilesTabWithComplexity(props: FilesTabWithComplexityProps) {
    ...
    function getEnvSpecificPrefix(filename: string): [boolean, string, string] {
        if (isBuildInProject && (filename.startsWith('builder.') || filename.startsWith('devservices.'))) {
        ...
        let rows = sortedFiles.map((file, rowIndex) => {
            ...
            return (
                <Tr key={file.name} style={{verticalAlign: "middle"}}>
                    ...
                    {!isBuildInProject && <Td modifier={"fitContent"}>
                        <FileComplexity file={file} complexity={complexity}/>
                    </Td>}
                    ...
                    <Table aria-label="Files" variant={"compact"} className={"files-table"} isStickyHeader>
                        <Thead>
                            <Tr>
                                ...
                                {!isBuildInProject && <Th key='complexity' width={20}>Complexity</Th>}
