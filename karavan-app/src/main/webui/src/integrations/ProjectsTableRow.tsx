...
import {ProjectStatusLabel} from "@/integrations/ProjectStatusLabel";
...
export function ProjectsTableRow(props: Props) {
    ...
    function getContainerStatus() {
        return containers.filter(c => c.projectId === project.projectId && ['devmode', 'packaged'].includes(c.type)).at(0);
    }
    ...
    return (
        <Tr key={project.projectId} style={{verticalAlign: "middle"}}>
            <Td modifier='fitContent' style={{paddingInlineEnd: 0, paddingInlineStart: '16px'}}>
                {!isBuildIn && container && <ProjectStatusLabel container={container}/>}
            </Td>
            <Td>
