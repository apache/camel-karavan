...
import {LANDSCAPE_FILE_NAME_JSON, LANDSCAPE_FILE_NAME_YAML} from "@core/contants";
...
export type CamelStatusName = 'context' | 'route' | 'processor' | 'consumer' | 'platform_http';
...
export function getProjectFileTypeByName(fileName: string): ProjectFileType[] {
    ...
    if (fileName.endsWith(".md")) return ProjectFileTypes.filter(p => p.name === "MARKDOWN")
    if (fileName.endsWith(".svg")) return ProjectFileTypes.filter(p => p.name === "SVG")
