...
export class LogWatchApi {
    ...
    static async fetchData(type: 'container' | 'build' | 'none', podName: string, controller: AbortController) {
        ...
        const fetchData = async () => {
            ...
            // The Authorization header is added by `authFetch` on every attempt so
            // that reconnects use a freshly refreshed token instead of replaying
            // the token captured when the stream was first opened.
            let attempt = 0;
            await fetchEventSource(url, {
                ...
                onmessage(event) {
                    if (event.event !== 'ping') {
                        LogsEventBus.sendLog('add', event.data);
                    } else {
