...
import {ProjectContainersContext} from "../context/ProjectContainersContextProvider";
...
export function DevModeToolbar() {
    ...
    const showLogDevMode = devModeContainerStatus && ['running', 'paused', 'exited'].includes(devModeContainerStatus?.state);
    ...
    const icon = isUp
        ? <UpIcon className={logIconClassName}/>
        : <DownIcon/>;
    ...
    function runDevMode(ev: any, verbose: boolean, compile: boolean = false) {
        ...
        ProjectService.startDevModeContainer(project, verbose, compile);
    }
    ...
    return (
        <div style={{display: 'flex', flexDirection: 'row', gap: '8px'}}>
            ...
            {isProjectContainer &&
                <Tooltip content={"Show runtime panel"} position={TooltipPosition.bottom}>
                    <Button className={"dev-action-button " + logButtonClassName}
                            variant={logButtonVariant}
                            icon={icon}
                        // isDisabled={!showLogDevMode}
                            onClick={e =>
                                setShowLog(true, 'container', packagedContainerStatuses?.[0].containerName)}>
                        <div style={{display: 'flex', alignItems: 'center', gap: '0.4rem'}}>
                            {packagedContainerStatuses?.[0].containerName}
                            <PackageIcon/>
                        </div>
                    </Button>
                </Tooltip>
            }
            {devModeContainerStatus?.containerId &&
                <Tooltip content={"Show runtime panel"} position={TooltipPosition.bottom}>
                    <Button className={"dev-action-button " + logButtonClassName}
                            variant={logButtonVariant}
                            icon={icon}
                            isDisabled={!showLogDevMode}
                            onClick={() => setShowLog(true, 'container', devModeContainerStatus.containerName)}>
                        <div style={{display: 'flex', alignItems: 'center', gap: '0.4rem'}}>
                            {devModeContainerStatus.containerName}
                            <DevIcon/>
                        </div>
                    </Button>
                </Tooltip>
            ...
            {!devModeIsRunning && !hasContainer() && !isProjectContainer && tabIndex !== 'package' &&
            ...
            {!devModeIsRunning && !hasContainer() && !isProjectContainer && tabIndex !== 'package' && getRunButton()}
