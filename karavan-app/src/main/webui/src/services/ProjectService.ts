...
import {CamelStatus, ContainerImage, ContainerStatus, Project, ProjectFile, ProjectType} from '@models/ProjectModels';
...
import {JSON_SCHEMA_EXTENSION, KARAVAN_DOT_EXTENSION, LANDSCAPE_FILE_NAME_JSON, OPENAPI_FILE_NAME_JSON} from "@core/contants";
import {useLogStore} from "@stores/LogStore";
import {useContainerStatusesStore} from "@stores/ContainerStatusesStore";
import {ComponentApi} from "@core/api/ComponentApi";

const COMPONENTS_BLACK_LIST = 'component-blocklist.txt';
...
export class ProjectService {
    ...
    public static startDevModeContainer(projectId: string, verbose: boolean, compile: boolean = false, showLog: boolean = true) {
        ...
        KaravanApi.startDevModeContainer(projectId, verbose, compile, res => {
            ...
            if (res.status === 200 || res.status === 201) {
                LogsEventBus.sendLog('set', '');
                useLogStore.setState({podName: res.data})
                useProjectStore.setState({tabIndex: 'log'})
            } else {
                const resData = (res as any)?.response?.data;
                const error = resData?.message ? resData?.message : res.statusText;
    ...
    public static loadBlockedComponentAndKamelets() {
        KaravanApi.getFiles(ProjectType.configuration, (files: ProjectFile[]) => {
            const c = files.find(f => f.name === COMPONENTS_BLACK_LIST);
    ...
    public static refreshCamelTraces(projectId: string) {
        KaravanApi.getProjectCamelTraces(projectId, res => {
            if (res.status === 200) {
