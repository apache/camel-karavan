...
export const useContainerStatusesStore = create<ContainerStatusesState>((set, get) => ({
    containers: [],
    ...
    fetchProjectContainers: async (projectId: string): Promise<void> => {
        ...
        await new Promise<AxiosResponse<ContainerStatus[]>>((resolve, reject) => {
            KaravanApi.getContainerStatus(projectId, (response: AxiosResponse<ContainerStatus[]>) => {
                resolve(response);
            });
        })
            .then((response) => {
