...
export const useProjectsStore = createWithEqualityFn<ProjectsState>((set, get) => ({
    ...
    setSelectedLabels: (selectedLabels: string[]) => {
        localStorage.setItem(STORAGE_KEY_PROJECTS_SELECTED_LABELS, JSON.stringify(selectedLabels));
        ...
        await new Promise<any>((resolve) => {
            ...
            const normalizedLabels = Object.fromEntries(
                Object.entries(projectLabels).map(([projectId, labels]) => {
                    ...
                    return [projectId, hasLabels ? labels : [PROJECT_WITH_NO_LABELS]];
                })
            );
            const labels = Array.from(new Set((Object.values(projectLabels) as string[][]).flat()));
            set({ projectLabels: normalizedLabels, labels: [...labels, PROJECT_WITH_NO_LABELS] });
        })
    },
...
export const useFilesStore = createWithEqualityFn<FilesState>((set, get) => ({
    ...
    fetchFile: async (projectId: string, fileName: string): Promise<ProjectFile> => {
        const currentFiles = get().files;
        return await new Promise<ProjectFile>((resolve) => {
            ...
            console.log(file)
            if (file !== undefined) {
                const oldFiles: ProjectFile[] = currentFiles.filter(f => f.name !== fileName);
    ...
    saveFile: async (file: ProjectFile, create: boolean = false) => {
        ...
        await new Promise<{ result: AxiosResponse | any; file: ProjectFile | any }>((resolve, reject) => {
            if (create) {
...
export const useFileStore = createWithEqualityFn<FileState>((set, get) => ({
    ...
    undoFile: async (): Promise<any> => {
        await new Promise<AxiosResponse>((resolve) => {
            ...
            if (result.status === 200 && !isEqual(get().fileCommited, get().file)) {
