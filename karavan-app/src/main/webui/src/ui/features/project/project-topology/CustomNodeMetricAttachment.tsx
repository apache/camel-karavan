...
export function CustomNodeMetricAttachment(props: Props) {
    ...
    const routeStatuses = (statusText ? JSON.parse(statusText) : {})?.route?.routes;
    const routeStatus = type === 'route'
        ? routeStatuses?.find((r: any) => r.routeId === routeId)
        : routeStatuses?.find((r: any) => r.from === "rest-openapi://classpath://openapi.json");
    ...
    return (
        <foreignObject x={-30} y={-9} width={120} height={75}>
