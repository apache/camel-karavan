...
export function useRouteDesignerHook() {
    ...
    const [setParentId, setShowSelector, setSelectorTabIndex, setParentDsl, setShowSteps, setSelectedPosition, routeId, setRouteId, isRouteTemplate, setIsRouteTemplate] = useCommandPaletteStore((s) =>
        [s.setParentId, s.setShowPalette, s.setSelectorTabIndex, s.setParentDsl, s.setShowSteps, s.setSelectedPosition, s.routeId, s.setRouteId,
    ...
    function onAddNewRouteStep(dsl: DslMetaModel, parentId: string, position?: number | undefined, fileName?: string) {
        switch (dsl.dsl) {
            case 'FromDefinition' :
                if (isRouteTemplate) {
    ...
    const createRoute = (dsl: DslMetaModel) => {
        const nodePrefixId = isKamelet() ? integration.metadata.name : 'route' + uuidv4().substring(0, 3);
        return CamelDefinitionApi.createRouteDefinition({
    ...
    return {
        deleteElement, selectElement, moveElement, onShowDeleteConfirmation, onAddNewRouteStep, openSelector,
