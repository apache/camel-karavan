...
import {getIntegrations} from "@page-project/project-topology/TopologyApi";
...
export const CAMEL_STEP_ELEMENTS: string[] = [
    "AggregateDefinition",
    "BeanDefinition",
    "ChoiceDefinition",
...
export class RouteToCreate {
    ...
    constructor(routeId: string, description: string, componentName: string, parameters: any) {
...
function getKeyWords() {
    if (KEY_WORDS.length === 0) {
        ...
        KEY_WORDS.length = 0;
        KEY_WORDS.push(...getWords(result.map(dsl => dsl.title)));
    }
    return KEY_WORDS;
}

function getWords(originalArray: string[]) {
    const allWords: string[] = originalArray.flatMap(item =>
        item
            .split(/[^a-zA-Z0-9.]+/) // Split by any character that isn't a letter, number, or dot
...
export class CamelUi {
    ...
    static createNewInternalRoute = (uri: string): RouteToCreate | undefined => {
        ...
        if (['direct', 'seda'].includes(componentName)) {
            parameters = {name: propertyValue};
        } else if ('vertx' === componentName) {
            parameters = {address: propertyValue};
        }
        ...
        return new RouteToCreate(routeId, description, componentName, parameters);
    }
    ...
    static createRouteFromComponent = (routeId: string, componentName: string, parameters: any = {}, expression: string = '[id: "Hello World"]', steps: CamelElement[] = []): RouteDefinition => {
        const stringWithParams = CamelUi.createFullStringWithParameters(componentName, parameters);
        const description = stringWithParams.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/[-_]/g, ' ').replace(/\b\w/g, char => char.toUpperCase());
        routeId = routeId ?? `route-${toSpecialRouteId(toKebabCase(stringWithParams))}`;
    ...
    static onCreateNewRoute = (projectId: string, routeId: string, files: ProjectFile[], componentName: string, parameters: any = {}, expression: string, after: (file: ProjectFile) => void): void => {
        try {
            ...
            if (incomingNodes.length > 0) {
                ...
                CamelUi.createNewRoute(projectId, routeId, componentName, parameters, expression, after);
            }
        } catch (err: any) {
            console.error(err)
        }
    ...
    static createNewRoute = (projectId: string, routeId: string, componentName: string, parameters: any = {}, expression: string, after: (file: ProjectFile) => void): void => {
        try {
            const stringWithParams = CamelUi.createFullStringWithParameters(componentName, parameters);
            const newRoute = CamelUi.createRouteFromComponent(routeId, componentName, parameters, expression);
    ...
    static keepOnlyWords = (inputString: string): string => {
        // Convert the array to a lowercase Set for fast, case-insensitive lookups
        const allowedSet = new Set(getKeyWords().map(word => word.toLowerCase()));

        return inputString
            .split(/\s+/) // Split the string into individual words by whitespace
            .filter(word => {
                // Strip punctuation to accurately check the word against your list
                const cleanWord = word.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
                // Keep the word only if it exists in your allowed list
                return allowedSet.has(cleanWord);
            })
            .join(' '); // Join the remaining words back with a single space
    }

    static checkFilter = (dsl: DslMetaModel, filter: string | undefined = undefined): boolean => {

        // Return early if there's no valid filter
        if (!filter || filter.trim() === "") {
            return true;
        }

        // Split the filter string into an array of words
        const words = filter.split(/[\s\p{P}]+/u).filter(Boolean);

        // If it's a single word, compare directly against name, title, or description
        if (words.length === 1) {
            const lowerFilter = words[0].toLowerCase();

            const name = dsl?.name?.toLowerCase() ?? "";
            const title = dsl?.title?.toLowerCase() ?? "";
            const description = dsl?.description?.toLowerCase() ?? "";
        ...
        // If it contains multiple words, pass the array to checkFilters
        return CamelUi.checkFilters(dsl, words);
    }

    static checkFilters = (dsl: DslMetaModel, filters: string[]): boolean => {
        // 1. Clean the array to only look at valid, non-empty filter strings
        const activeFilters = filters.filter(f => f !== undefined && f !== "");

        // 2. If no valid filters are provided, return true (same as original logic)
        if (activeFilters.length === 0) {
            return true;
        }

        // 3. Extract and lowercase all searchable fields once for better performance
        const name = dsl?.name?.toLowerCase() ?? "";
        const description = dsl?.description?.toLowerCase() ?? "";
        const title = dsl?.title?.toLowerCase() ?? "";

        // 4. Return true if ANY active filter matches ANY of the fields
        ...
        if (!parentDsl) {
            ...
            if (showSteps) {
                if (parentDsl && CamelDefinitionApiExt.getElementChildrenDefinition(parentDsl).filter(child => child.name === 'steps').length > 0) {
                    CAMEL_STEP_ELEMENTS.forEach(se => {
    ...
    static getFlowCounts = (i: Integration): Map<string, number> => {
        ...
        result.set('templatedRoutes', i.spec.flows?.filter((e: any) => ['TemplatedRouteDefinition'].includes(e.dslName)).length || 0);
