...
const editorOptions: monaco.editor.IStandaloneEditorConstructionOptions = {
    ...
    scrollBeyondLastLine: false, // Crucial so height maps exactly to content
    scrollbar: {
        useShadows: false,
        vertical: 'hidden',   // Hide scrollbars since we auto-expand height
        horizontal: 'hidden'
    },
    ...
    wordWrap: "on", // Ensures long single-line strings trigger height expansion
    // 1. Disables suggesting words already present in the document
    wordBasedSuggestions: "off",

    // 2. Disables the popup from appearing automatically as you type
    quickSuggestions: false,

    // 3. Disables suggesting snippets
    snippetSuggestions: "none",

    // 4. Disables the popup when typing trigger characters (like a dot or space)
    suggestOnTriggerCharacters: false,

    // Your existing suggest block can stay as a fallback
    suggest: {
        showKeywords: false,
        showStatusBar: false,
        showIcons: false,
...
export function KameletPropertyField(props: Props) {
    ...
    function getMonacoInput() {
        ...
        return (
            <div className={"kamelet-monaco-input"}>
                <MonacoEditorWrapper
        ...
        <div>
            <FormGroup
                ...
                {property.type === 'string'
                    && property.format === 'monaco' && getMonacoInput()}
                {['string', 'integer', 'int', 'number'].includes(property.type)
                    && property?.format !== "monaco"
                    && getSpecialStringInput()}
