...
import {DslPropertyFieldSelect} from "./DslPropertyFieldSelect";
import {MEDIA_TYPES} from "../../utils/MediaTypes";
import {DslPropertyFieldSelectScrollable} from "./DslPropertyFieldSelectScrollable";
...
export function DslPropertyField(props: Props) {
    ...
    const [multiObjectIndex, setMultiObjectIndex] = useState<number>();
    ...
    useEffect(() => {
        if (stepDoubleClicked && selectedStep) {
            ...
            // const value = dslLanguage ? (selectedStep as any)?.expression?.[dslLanguage?.[0]]?.expression : undefined;
            if (dsp?.propertyName === property.name) {
                if (dsp.isExpression && dslLanguage && !value?.startsWith("resource:")) {
                    setConfigurationSelectorDefaultTab(dsp.tab)
                    openConfigurationSelector(property.name);
                } else if (value !== undefined && typeof value === 'string' && value?.startsWith("resource:")) {
    ...
    function getBooleanInput(property: PropertyMeta) {
        const id = property.name + "-boolean-value";
        ...
        const canBePlaceholder = selectedStep?.dslName !== 'RouteTemplateDefinition';
        return (
            <InputGroup className={"input-group " + valueChangedClassName} key={id}>
                <InputGroupItem>
                    {isChecked}
                    <Checkbox
                        ref={ref}
    ...
    function getMultiObjectFieldProps(property: PropertyMeta, value: any, v: any, index: number, hideLabel: boolean = false) {
        return (<>
            <div className="object" key={`object-${property.name}-${index}`}>
