...
export function ProjectLabelRoutes(props: Props) {
    ...
    if (complexity?.routes?.length > 20) {
        status = "danger";
        color = "var(--pf-t--global--icon--color--status--danger--default)"
        icon = <ExclamationCircleIcon color={color}/>
    } else if (complexity?.routes?.length > 10) {
        status = "warning";
    ...
    const compact = status === "success"
        ? icon
        : <Tooltip content={`${complexity?.routes?.length} routes`}>
            {icon}
        </Tooltip>

    return full === true
        ? <Tooltip content={`Project contains ${complexity?.routes?.length} routes`} position="left">
            <Label status={status} isCompact variant={'outline'}>{complexity?.routes?.length}</Label>
        </Tooltip>
        : compact
}