...
import {CheckIcon, CopyIcon, DownloadIcon, EditIcon, OutlinedCopyIcon, SearchIcon, ShareAltIcon, TimesIcon} from '@patternfly/react-icons';
...
import {FileCopyForEnvModal} from "../files/FileCopyForEnvModal";
import {CamelUtil} from "@core/api/CamelUtil";
import {download, getIcon, sortFiles} from "../files/FilesTabUtils";
...
export function FilesSubTab(props: FilesSubTabProps) {
    ...
    function shareRoute() {
        if (project.projectId && fileNameToShare && openShare !== undefined) {
            const target = ProjectType.kamelets;
            KaravanApi.copyProjectFile(project.projectId, fileNameToShare, target, fileNameToShare, true, res => {
    ...
    function canDeleteFiles(filename: string): boolean {
        if (deletedFilenames.includes(filename)) {
            return false;
        } else if (project.projectId === ProjectType.templates.toString() && !filename.startsWith(LANDSCAPE_TEMPLATE_FILENAME_PREFIX)) {
            return false;
    ...
    function getTableBody() {
        ...
        const rows = sortedFiles.map((file, rowIndex) => {
            ...
            const hasRouteTemplate = CamelUtil.hasRouteTemplateDefinitions(file.name, file.code);
                ...
                <Tr key={file.name} style={{verticalAlign: "middle"}}>
                    ...
                    <Td textCenter>
                        {needCommit(file.name) &&
                            <Label color="grey" icon={<OutlinedCopyIcon/>}>{diffType}</Label>
                        }
                        {!needCommit(file.name) &&
                    ...
                    <Td modifier={"fitContent"}>
                        <Flex direction={{default: "row"}} justifyContent={{default: "justifyContentFlexEnd"}} spaceItems={{default: 'spaceItemsNone'}}
                            ...
                            <Button className="dev-action-button"
                                    variant={(!isDev || !canDeleteFiles(file.name)) ? "plain" : "link"}
                                    isDanger
                                    ...
                                    isDisabled={!isDev || !canDeleteFiles(file.name)}
                                    isInline={!isDev}
                                    onClick={e =>
                                        setFile('delete', file)
                                    }>
