...
import {ProjectPageSideBarType, useProjectPageStore} from "../ProjectPageStore";
...
export function useTopologyHook() {
    ...
    const setTabIndex = useProjectStore(state => state.setTabIndex);
    const [setFile] = useFileStore((s) => [s.setFile], shallow);
    const [files] = useFilesStore((s) => [s.files], shallow);
    const {setShowSideBar, setTitle} = useProjectPageStore();
    const {setConfirmationProps} = useArchitectureStore();

    function showSideBar(showSideBar: ProjectPageSideBarType, selectedId: string, title: string) {
        setShowSideBar(showSideBar)
        setTitle(`${title}: ${capitalize(showSideBar)}`)
    }
    ...
    return {
        selectFile, setDisabled, deleteRoute, setRouteGroup, project, showSideBar, files
    }
}