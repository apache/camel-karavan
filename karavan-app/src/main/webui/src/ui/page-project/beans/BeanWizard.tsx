...
import {useFilesStore, useFileStore, useProjectStore, useWizardStore} from "@stores/ProjectStore";
...
import {KARAVAN_DOT_EXTENSION, WIZARD_BEAN_TEMPLATE_FILENAME_PREFIX} from "@core/contants";
...
export function BeanWizard() {
    ...
    useEffect(() => {
        const templateNames = templateFiles?.filter(f => f.name.startsWith(WIZARD_BEAN_TEMPLATE_FILENAME_PREFIX))
            .map(file => file.name.replace(WIZARD_BEAN_TEMPLATE_FILENAME_PREFIX, '').replace(KARAVAN_DOT_EXTENSION.CAMEL_YAML, ''));
    ...
    function handleFormSubmit() {
        ...
        if (file && bean !== undefined) {
            ...
            const fullFileName = filename + KARAVAN_DOT_EXTENSION.CAMEL_YAML;
    ...
    function getBeanFactoryDefinitions(): BeanFactoryDefinition[] {
        const fs = templateFiles?.filter(f => f.name === WIZARD_BEAN_TEMPLATE_FILENAME_PREFIX.concat(templateName).concat(KARAVAN_DOT_EXTENSION.CAMEL_YAML));
        ...
        <Modal onClose={_ => setShowWizard(false)}
            ...
            <ModalBody>
                <Wizard className="bean-wizard" height={600} onClose={() => setShowWizard(false)} onSubmit={event => handleFormSubmit()}>
                    ...
                    >
                        <Flex direction={{default: "column"}} gap={{default: 'gapLg'}}>
                            <Radio key={EMPTY_BEAN}
                                   id={EMPTY_BEAN}
                                   label={capitalize(EMPTY_BEAN)}
                                   name={EMPTY_BEAN}
                                   aria-label={EMPTY_BEAN}
                                   isChecked={EMPTY_BEAN === templateName}
                                   onChange={_ => setTemplateName(EMPTY_BEAN)}
                            />
                            {templateNames.map(n => <Radio key={n} aria-label={n} id={n} label={capitalize(n)} name={n}
                    ...
                    >
                        <Flex direction={{default: "column"}} gap={{default: 'gapLg'}}>
                            {getBeans.map(b => <Radio key={b.name} aria-label={b.name} id={b.name} label={b.name} name={b.name}
                                ...
                                <InputGroup>
                                    <InputGroupItem isFill>
                                        <TextInput className="text-field" type="text" id="filename"
                                                   aria-label="filename"
                                                   value={filename}
                                                   customIcon={<Content component="p">{KARAVAN_DOT_EXTENSION.CAMEL_YAML}</Content>}
                                                   onChange={(e, value) => {
                                                       setFilename(value);
                                                   }}
                                        />
