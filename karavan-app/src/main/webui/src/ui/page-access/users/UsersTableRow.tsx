...
import {PauseIcon, PlayIcon, TimesIcon, UserSecretIcon} from "@patternfly/react-icons";
...
export function UsersTableRow(props: Props) {
    ...
    const {setShowUserModal, setCurrentUser, roles, setShowPasswordModal, refreshAccess} = useAccessStore();
    ...
    function executeAction() {
        if (command === 'delete') {
            ...
            AccessApi.setUserStatus(user, status).then(value => refreshAccess());
        } else if (command && ['add', 'remove'].includes(command)) {
            AccessApi.setUserRole(user, role, command).then(value => refreshAccess());
        }
