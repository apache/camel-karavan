...
interface Props {
    ...
    onAfterCreate: (configmapKey: ConfigMapKey) => void
    onCancel: () => void
}

export function ConfigMapKeyModal(props: Props) {

    const [isReset, setReset] = React.useState(false);
    const formContext = useForm<ConfigMapKey>({mode: "all"});
    ...
    const onSubmit: SubmitHandler<ConfigMapKey> = (configmapKey) => {
        SystemApi.setConfigMapValue(configmapName, configmapKey.key, configmapKey.value, (val: string) => {
            SystemService.refresh();
            reset();
            props.onAfterCreate(configmapKey);
        });
    }
    ...
    const {configmapName, isOpen, onCancel} = props;
    ...
    return (
        <Modal
            ...
            <ModalHeader title={"Create key for " + props.configmapName}/>
            <ModalBody>
