...
* Run using CLI
    ```shell
    jbang -Dcamel.jbang.version=4.14.5 camel@apache/camel run $INTEGRATION.yaml --max-messages=10 --logging-level=info
    ```
...
* Export using CLI
    ```shell
    jbang -Dcamel.jbang.version=4.14.5 camel@apache/camel export --directory=export
    ```
