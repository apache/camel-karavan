...
public class ProjectStarter implements HealthCheck {
    ...
    public void tryStart() throws Exception {
        ...
        if (gitService.checkGit()) {
            if (projectsCache.getProjects().isEmpty()) {
                importAllProjects();
            }
    ...
    void addKameletsProject() {
        ...
        try {
            Project kamelets = projectsCache.getProject(Project.Type.kamelets.name());
            if (kamelets == null) {
                ...
                projectsCache.saveProject(kamelets);
            }
        } catch (Exception e) {
    ...
    void addTemplatesProject() {
        ...
        try {
            ...
            if (templates == null) {
                ...
                codeService.getTemplates().forEach((name, value) -> {
                    ProjectFile f = projectsCache.getProjectFile(Project.Type.templates.name(), name);
                    if (f == null) {
                        ProjectFile file = new ProjectFile(name, value, Project.Type.templates.name(), Instant.now().toEpochMilli());
                        projectsCache.saveProjectFile(file);
                    }
    ...
    void addServicesProject() {
        ...
        try {
            Project services = projectsCache.getProject(Project.Type.services.name());
            if (services == null) {
                ...
                projectsCache.saveProject(services);
