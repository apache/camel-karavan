...
import org.apache.camel.karavan.project.model.GitRepoFile;
...
public class CodeService {
    ...
    public Map<String, String> getProjectFilesForDevMode(String projectId, Boolean withKamelets) {
        Map<String, String> files = projectsCache.getProjectFiles(projectId).stream()
        ...
        if (withKamelets) {
            projectsCache.getProjectFiles(Project.Type.kamelets.name())
    ...
    public List<Tuple3<String, String, String>> getBuilderEnvMapping() {
        ...
        ProjectFile projectFile = projectsCache.getProjectFile(Project.Type.templates.name(), BUILDER_ENV_MAPPING_FILENAME);
        if (projectFile != null) {
    ...
    public String getTemplateText(String fileName) {
        try {
            List<ProjectFile> files = projectsCache.getProjectFiles(Project.Type.templates.name());
...
//            }
    ...
    public Integer getProjectPort(String projectId) {
        ProjectFile composeFile = projectsCache.getProjectFile(projectId, PROJECT_COMPOSE_FILENAME);
        return getProjectPort(composeFile);
    }


    public DockerComposeService getDockerComposeService(String projectId) {
        ProjectFile compose = projectsCache.getProjectFile(projectId, PROJECT_COMPOSE_FILENAME);
        if (compose != null) {
    ...
    public void updateDockerComposeImage(String projectId, String imageName) {
        ...
        if (compose != null) {
            ...
            projectsCache.saveProjectFile(compose);
        }
    }
