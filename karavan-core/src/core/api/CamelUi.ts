...
import {CamelMetadataApi, CamelModelMetadata, ElementMeta} from "../model/CamelMetadata";
...
export class CamelUi { // TODO: Move class to karavan-designer ?
    ...
    static getTitle = (element: CamelElement): string => {
        ...
        if (k) {
            ...
            const uri: string = (element as any).uri;
            return uri
                ? CamelUtil.capitalizeName((element as any).stepName) + " : " + ComponentApi.getComponentNameFromUri(uri)
    ...
    static getIcon = (element: CamelElement): string => {
        const k: KameletModel | undefined = CamelUi.getKamelet(element);
        if (["FromDefinition", "FromDefinition", "KameletDefinition"].includes(element.dslName)) {
