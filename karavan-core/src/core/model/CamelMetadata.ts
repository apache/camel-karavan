...
export const CamelModelMetadata: ElementMeta[] = [
    ...
    new ElementMeta('restConfiguration', 'RestConfigurationDefinition', 'Rest Configuration', "To configure rest", 'rest', [
        ...
        new PropertyMeta('apiComponent', 'Api Component', "The name of the Camel component to use as the REST API. If no API Component has been explicit configured, then Camel will lookup if there is a Camel component responsible for servicing and generating the REST API documentation, or if a org.apache.camel.spi.RestApiProcessorFactory is registered in the registry. If either one is found, then that is being used.", 'string', '', '', false, false, false, false, 'consumer,advanced'),
        new PropertyMeta('producerComponent', 'Producer Component', "Sets the name of the Camel component to use as the REST producer", 'string', '', '', false, false, false, false, 'producer,advanced'),
        ...
        new PropertyMeta('apiContextRouteId', 'Api Context Route Id', "Sets the route id to use for the route that services the REST API. The route will by default use an auto assigned route id.", 'string', '', '', false, false, false, false, 'consumer'),
    ...
    new ElementMeta('faultToleranceConfiguration', 'FaultToleranceConfigurationDefinition', 'Fault Tolerance Configuration', "MicroProfile Fault Tolerance Circuit Breaker EIP configuration", 'configuration,eip', [
        new PropertyMeta('circuitBreakerRef', 'Circuit Breaker Ref', "Refers to an existing io.smallrye.faulttolerance.core.circuit.breaker.CircuitBreaker instance to lookup and use from the registry. When using this, then any other circuit breaker options are not in use.", 'string', '', '', false, false, false, false, 'circuitbreaker'),
        ...
        new PropertyMeta('timeoutScheduledExecutorServiceRef', 'Timeout Scheduled Executor Service Ref', "References to a custom thread pool to use when timeout is enabled", 'string', '', '', false, false, false, false, 'timeout'),
        ...
        new PropertyMeta('bulkheadExecutorServiceRef', 'Bulkhead Executor Service Ref', "References to a custom thread pool to use when bulkhead is enabled.", 'string', '', '', false, false, false, false, 'bulkhead'),
        new PropertyMeta('id', 'Id', "The id of this node", 'string', '', '', false, false, false, false, ''),
    ]),
    ...
    new ElementMeta('resilience4jConfiguration', 'Resilience4jConfigurationDefinition', 'Resilience4j Configuration', "Resilience4j Circuit Breaker EIP configuration", 'configuration,eip', [
        new PropertyMeta('circuitBreakerRef', 'Circuit Breaker Ref', "Refers to an existing io.github.resilience4j.circuitbreaker.CircuitBreaker instance to lookup and use from the registry. When using this, then any other circuit breaker options are not in use.", 'string', '', '', false, false, false, false, 'circuitbreaker'),
        new PropertyMeta('configRef', 'Config Ref', "Refers to an existing io.github.resilience4j.circuitbreaker.CircuitBreakerConfig instance to lookup and use from the registry.", 'string', '', '', false, false, false, false, 'circuitbreaker'),
    ...
    new ElementMeta('rest', 'RestDefinition', 'Rest', "Defines a rest service using the rest-dsl", 'rest', [
        ...
        new PropertyMeta('securityRequirements', 'Security Requirements', "Sets the security requirement(s) for all endpoints.", 'RestSecuritiesRequirement', '', '', false, false, false, true, 'security'),
        new PropertyMeta('id', 'Id', "Sets the id of this node", 'string', '', '', false, false, false, false, ''),
        new PropertyMeta('description', 'Description', "Sets the description of this node", 'string', '', '', false, false, false, false, ''),
    ...
    new ElementMeta('hystrixConfiguration', 'HystrixConfigurationDefinition', 'Hystrix Configuration', "Hystrix Circuit Breaker EIP configuration", 'configuration,eip', [
        ...
        new PropertyMeta('circuitBreakerEnabled', 'Circuit Breaker Enabled', "Whether to use a HystrixCircuitBreaker or not. If false no circuit-breaker logic will be used and all requests permitted. This is similar in effect to circuitBreakerForceClosed() except that continues tracking metrics and knowing whether it should be open/closed, this property results in not even instantiating a circuit-breaker.", 'boolean', '', 'true', false, false, false, false, 'command'),
        new PropertyMeta('circuitBreakerErrorThresholdPercentage', 'Circuit Breaker Error Threshold Percentage', "Error percentage threshold (as whole number such as 50) at which point the circuit breaker will trip open and reject requests. It will stay tripped for the duration defined in circuitBreakerSleepWindowInMilliseconds; The error percentage this is compared against comes from HystrixCommandMetrics.getHealthCounts().", 'number', '', '50', false, false, false, false, 'command'),
        new PropertyMeta('circuitBreakerForceClosed', 'Circuit Breaker Force Closed', "If true the HystrixCircuitBreaker#allowRequest() will always return true to allow requests regardless of the error percentage from HystrixCommandMetrics.getHealthCounts(). The circuitBreakerForceOpen() property takes precedence so if it set to true this property does nothing.", 'boolean', '', 'false', false, false, false, false, 'command'),
        new PropertyMeta('circuitBreakerForceOpen', 'Circuit Breaker Force Open', "If true the HystrixCircuitBreaker.allowRequest() will always return false, causing the circuit to be open (tripped) and reject all requests. This property takes precedence over circuitBreakerForceClosed();", 'boolean', '', 'false', false, false, false, false, 'command'),
        new PropertyMeta('circuitBreakerRequestVolumeThreshold', 'Circuit Breaker Request Volume Threshold', "Minimum number of requests in the metricsRollingStatisticalWindowInMilliseconds() that must exist before the HystrixCircuitBreaker will trip. If below this number the circuit will not trip regardless of error percentage.", 'number', '', '20', false, false, false, false, 'command'),
        new PropertyMeta('circuitBreakerSleepWindowInMilliseconds', 'Circuit Breaker Sleep Window In Milliseconds', "The time in milliseconds after a HystrixCircuitBreaker trips open that it should wait before trying requests again.", 'number', '', '5000', false, false, false, false, 'command'),
        new PropertyMeta('executionIsolationSemaphoreMaxConcurrentRequests', 'Execution Isolation Semaphore Max Concurrent Requests', "Number of concurrent requests permitted to HystrixCommand.run(). Requests beyond the concurrent limit will be rejected. Applicable only when executionIsolationStrategy == SEMAPHORE.", 'number', '', '20', false, false, false, false, 'command'),
        new PropertyMeta('executionIsolationStrategy', 'Execution Isolation Strategy', "What isolation strategy HystrixCommand.run() will be executed with. If THREAD then it will be executed on a separate thread and concurrent requests limited by the number of threads in the thread-pool. If SEMAPHORE then it will be executed on the calling thread and concurrent requests limited by the semaphore count.", 'string', 'THREAD, SEMAPHORE', 'THREAD', false, false, false, false, 'command'),
        new PropertyMeta('executionIsolationThreadInterruptOnTimeout', 'Execution Isolation Thread Interrupt On Timeout', "Whether the execution thread should attempt an interrupt (using Future#cancel ) when a thread times out. Applicable only when executionIsolationStrategy() == THREAD.", 'boolean', '', 'true', false, false, false, false, 'command'),
        new PropertyMeta('executionTimeoutInMilliseconds', 'Execution Timeout In Milliseconds', "Time in milliseconds at which point the command will timeout and halt execution. If executionIsolationThreadInterruptOnTimeout == true and the command is thread-isolated, the executing thread will be interrupted. If the command is semaphore-isolated and a HystrixObservableCommand, that command will get unsubscribed.", 'number', '', '1000', false, false, false, false, 'command'),
        new PropertyMeta('executionTimeoutEnabled', 'Execution Timeout Enabled', "Whether the timeout mechanism is enabled for this command", 'boolean', '', 'true', false, false, false, false, 'command'),
        new PropertyMeta('fallbackIsolationSemaphoreMaxConcurrentRequests', 'Fallback Isolation Semaphore Max Concurrent Requests', "Number of concurrent requests permitted to HystrixCommand.getFallback(). Requests beyond the concurrent limit will fail-fast and not attempt retrieving a fallback.", 'number', '', '10', false, false, false, false, 'command'),
        new PropertyMeta('fallbackEnabled', 'Fallback Enabled', "Whether HystrixCommand.getFallback() should be attempted when failure occurs.", 'boolean', '', 'true', false, false, false, false, 'command'),
        new PropertyMeta('metricsHealthSnapshotIntervalInMilliseconds', 'Metrics Health Snapshot Interval In Milliseconds', "Time in milliseconds to wait between allowing health snapshots to be taken that calculate success and error percentages and affect HystrixCircuitBreaker.isOpen() status. On high-volume circuits the continual calculation of error percentage can become CPU intensive thus this controls how often it is calculated.", 'number', '', '500', false, false, false, false, 'command'),
        new PropertyMeta('metricsRollingPercentileBucketSize', 'Metrics Rolling Percentile Bucket Size', "Maximum number of values stored in each bucket of the rolling percentile. This is passed into HystrixRollingPercentile inside HystrixCommandMetrics.", 'number', '', '10', false, false, false, false, 'command'),
        new PropertyMeta('metricsRollingPercentileEnabled', 'Metrics Rolling Percentile Enabled', "Whether percentile metrics should be captured using HystrixRollingPercentile inside HystrixCommandMetrics.", 'boolean', '', 'true', false, false, false, false, 'command'),
        new PropertyMeta('metricsRollingPercentileWindowInMilliseconds', 'Metrics Rolling Percentile Window In Milliseconds', "Duration of percentile rolling window in milliseconds. This is passed into HystrixRollingPercentile inside HystrixCommandMetrics.", 'number', '', '10000', false, false, false, false, 'command'),
        new PropertyMeta('metricsRollingPercentileWindowBuckets', 'Metrics Rolling Percentile Window Buckets', "Number of buckets the rolling percentile window is broken into. This is passed into HystrixRollingPercentile inside HystrixCommandMetrics.", 'number', '', '6', false, false, false, false, 'command'),
        new PropertyMeta('metricsRollingStatisticalWindowInMilliseconds', 'Metrics Rolling Statistical Window In Milliseconds', "This property sets the duration of the statistical rolling window, in milliseconds. This is how long metrics are kept for the thread pool. The window is divided into buckets and rolls by those increments.", 'number', '', '10000', false, false, false, false, 'command'),
        new PropertyMeta('metricsRollingStatisticalWindowBuckets', 'Metrics Rolling Statistical Window Buckets', "Number of buckets the rolling statistical window is broken into. This is passed into HystrixRollingNumber inside HystrixCommandMetrics.", 'number', '', '10', false, false, false, false, 'command'),
        new PropertyMeta('requestLogEnabled', 'Request Log Enabled', "Whether HystrixCommand execution and events should be logged to HystrixRequestLog.", 'boolean', '', 'true', false, false, false, false, 'command'),
        new PropertyMeta('corePoolSize', 'Core Pool Size', "Core thread-pool size that gets passed to java.util.concurrent.ThreadPoolExecutor#setCorePoolSize(int)", 'number', '', '10', false, false, false, false, 'threadpool'),
        new PropertyMeta('maximumSize', 'Maximum Size', "Maximum thread-pool size that gets passed to ThreadPoolExecutor#setMaximumPoolSize(int) . This is the maximum amount of concurrency that can be supported without starting to reject HystrixCommands. Please note that this setting only takes effect if you also set allowMaximumSizeToDivergeFromCoreSize", 'number', '', '10', false, false, false, false, 'threadpool'),
        new PropertyMeta('keepAliveTime', 'Keep Alive Time', "Keep-alive time in minutes that gets passed to ThreadPoolExecutor#setKeepAliveTime(long,TimeUnit)", 'number', '', '1', false, false, false, false, 'threadpool'),
        new PropertyMeta('maxQueueSize', 'Max Queue Size', "Max queue size that gets passed to BlockingQueue in HystrixConcurrencyStrategy.getBlockingQueue(int) This should only affect the instantiation of a threadpool - it is not eliglible to change a queue size on the fly. For that, use queueSizeRejectionThreshold().", 'number', '', '-1', false, false, false, false, 'threadpool'),
        new PropertyMeta('queueSizeRejectionThreshold', 'Queue Size Rejection Threshold', "Queue size rejection threshold is an artificial max size at which rejections will occur even if maxQueueSize has not been reached. This is done because the maxQueueSize of a BlockingQueue can not be dynamically changed and we want to support dynamically changing the queue size that affects rejections. This is used by HystrixCommand when queuing a thread for execution.", 'number', '', '5', false, false, false, false, 'threadpool'),
        new PropertyMeta('threadPoolRollingNumberStatisticalWindowInMilliseconds', 'Thread Pool Rolling Number Statistical Window In Milliseconds', "Duration of statistical rolling window in milliseconds. This is passed into HystrixRollingNumber inside each HystrixThreadPoolMetrics instance.", 'number', '', '10000', false, false, false, false, 'threadpool'),
        new PropertyMeta('threadPoolRollingNumberStatisticalWindowBuckets', 'Thread Pool Rolling Number Statistical Window Buckets', "Number of buckets the rolling statistical window is broken into. This is passed into HystrixRollingNumber inside each HystrixThreadPoolMetrics instance.", 'number', '', '10', false, false, false, false, 'threadpool'),
        new PropertyMeta('allowMaximumSizeToDivergeFromCoreSize', 'Allow Maximum Size To Diverge From Core Size', "Allows the configuration for maximumSize to take effect. That value can then be equal to, or higher, than coreSize", 'boolean', '', 'false', false, false, false, false, 'threadpool'),
        new PropertyMeta('id', 'Id', "The id of this node", 'string', '', '', false, false, false, false, ''),
    ]),
]
